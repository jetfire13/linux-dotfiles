...
void manage(Window w, XWindowAttributes *wa)
{
    ...
    c->y = c->mon->my + (c->mon->mh - HEIGHT(c)) / 2;
    {
        ...
        if (XGetWindowProperty(dpy, c->win, netatom[NetClientInfo], 0L, 2L, False, XA_CARDINAL, &atom, &format, &n,
                               &extra, (unsigned char **)&data) == Success &&
            n == 2)
        {
            c->tags = *data;
            for (m = mons; m; m = m->next)
...
void resizeclient(Client *c, int x, int y, int w, int h)
{
    ...
    wc.border_width = c->bw;
    if (!selmon->pertag
             ->drawwithgaps[selmon->pertag->curtag] && /* this is the noborderfloatingfix patch, slightly modified so
                                                          that it will work if, and only if, gaps are disabled. */
        (((nexttiled(c->mon->clients) == c &&
           !nexttiled(c->next)) /* these two first lines are the only ones changed. if you are manually patching and
                                   have noborder installed already, just change these lines; or conversely, just remove
                                   this section if the noborder patch is not desired;) */
...
void tile(Monitor *m)
{
    ...
    { /* draw with fullgaps logic */
        if (n > m->nmaster)
            mw = m->nmaster ? m->ww * m->mfact : 0;
        else
            mw = m->ww - m->pertag->gappx[m->pertag->curtag];
        for (i = 0, my = ty = m->pertag->gappx[m->pertag->curtag], c = nexttiled(m->clients); c;
    ...
    { /* draw with singularborders logic */
        if (n > m->nmaster)
            mw = m->nmaster ? m->ww * m->mfact : 0;
        else
            mw = m->ww;
...
void window_set_state(Display *dpy, Window win, long state)
{
    ...
    XChangeProperty(dpy, win, wmatom[WMState], wmatom[WMState], 32, PropModeReplace, (unsigned char *)data, 2);
}
